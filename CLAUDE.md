...
- Player has real `idle` (5 frames), `run` (7 frames), `jump` (10 frames), and `fall` (2 frames) animations so far (`src/assets/sprites/player/idle1..5.png`, `run_1..7.png`, `jump_1..10.png`, `fall_3.png`+`fall_4.png`); hurt frames and everything else (enemy, checkpoint, goal, collectible, tileset, background, HUD icons) are still the placeholder shapes described above. See `ASSET_SPEC.md` for the full remaining shopping list.
