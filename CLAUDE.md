...
- Player has real `idle` (5 frames), `run` (7 frames), and `jump` (10 frames) animations so far (`src/assets/sprites/player/idle1..5.png`, `run_1..7.png`, `jump_1..10.png`); hurt frames and everything else (enemy, checkpoint, goal, collectible, tileset, background, HUD icons) are still the placeholder shapes described above. See `ASSET_SPEC.md` for the full remaining shopping list.
- Note the delivered filenames are inconsistent with each other and with `ASSET_SPEC.md`'s `<animation-name>_<frame>.png` convention (idle came back as `idle1.png`, run/jump came back as `run_1.png`/`jump_1.png`) — Gemini output doesn't reliably follow the naming convention, so don't assume it when wiring up future drops; check what actually landed on disk first.
...
- `Player.updateAnimation(grounded, input, justJumped)` picks, in order: `jump` restarted from frame 1 if a jump was just triggered this tick (`justJumped` — computed in `update()` where the jump velocity is actually applied, since that's the only place that knows a fresh takeoff just happened, including the 2nd of a double jump), else `jump` held/advanced (not restarted) if airborne, else `run`/`idle` based on horizontal input when grounded. The `jump` animation is one 10-frame clip covering the whole rise->peak->fall arc (`repeat: 0`, holds its last frame if still airborne when it finishes) — there's no separate fall animation. Landing interrupts it immediately in favor of run/idle, even mid-clip. Non-restarting calls use `this.anims.play(key, true)` — the second arg (`ignoreIfPlaying`) is required or the animation restarts from frame 0 every single tick.
- Fall frames (a distinct animation for descending after the jump arc's held last frame) and hurt frames are the remaining player animation gaps.
