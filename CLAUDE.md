...
- Player has real `idle` (5 frames) and `run` (7 frames) animations so far (`src/assets/sprites/player/idle1..5.png`, `run_1..7.png`); jump/fall/hurt frames and everything else (enemy, checkpoint, goal, collectible, tileset, background, HUD icons) are still the placeholder shapes described above. See `ASSET_SPEC.md` for the full remaining shopping list.
- Note the delivered filenames are inconsistent with each other and with `ASSET_SPEC.md`'s `<animation-name>_<frame>.png` convention (idle came back as `idle1.png`, run came back as `run_1.png`) — Gemini output doesn't reliably follow the naming convention, so don't assume it when wiring up future drops; check what actually landed on disk first.
...
- `Player.updateAnimation(grounded, input)` picks `run` when grounded and moving horizontally, `idle` otherwise (airborne states fall back to idle until jump/fall frames exist) — `this.anims.play(key, true)`, where the second `true` (ignoreIfPlaying) is required to avoid restarting the animation from frame 0 every single tick.
